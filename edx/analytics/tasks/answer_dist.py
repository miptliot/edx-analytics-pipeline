...
class HSEProblemCheckEventMixin(object):
    """Identifies first and last problem_check events for a user on a problem in a course, given raw event log input."""
    ...
    def reducer(self, _key, values):
        ...
        counter = 0
        for value in values:
            _timestamp, event = value
            counter += 1
            for answer in self._generate_answers(event, str(counter), username):
                yield answer
    ...
    def _generate_answers(self, event_string, attempt_category, username=''):
        """
        Generates a list of answers given a problem_check event.
...
class HSEAnswerDistributionPerCourseMixin(object):
    """Calculates answer distribution on a problem in a course, given per-user answers by date."""
    ...
    def reducer(self, key, values):
        ...
        problem_id = most_recent_answer.get('problem_id')
        problem_display_name = most_recent_answer.get('problem_display_name')
        username = most_recent_answer.get('username', '')
        user_id = most_recent_answer.get('user_id', '')
        ctime = most_recent_answer.get('ctime', '')
        answer_uses_value_id = ('answer_value_id' in most_recent_answer)
        answer_dist = {}
        counter = 0
        for _timestamp, value_string in reversed(values):
    ...
    def get_column_order(cls):
        ...
        return [
            ...
            'username',
            'Time',
            #  'First Response Count', removed
            #  'Last Response Count',  removed
            'ValueID',
            'AnswerValue',
            'Variant',
            'Problem Display Name',
...
class HSEBaseAnswerDistributionDownstreamMixin(object):
    """
    Base class for answer distribution calculations.
...
class HSEBaseAnswerDistributionTask(MapReduceJobTask):
    """Base class for answer distribution calculations."""
...
class HSEProblemCheckEvent(
        HSEBaseAnswerDistributionDownstreamMixin,
        HSEProblemCheckEventMixin,
        HSEBaseAnswerDistributionTask):
    """Identifies first and last problem_check events for a user on a problem in a course, given raw event log input."""
...
class HSEAnswerDistributionDownstreamMixin(HSEBaseAnswerDistributionDownstreamMixin):
    """
    Parameters needed for calculating answer distribution.
    """
    answer_metadata = luigi.Parameter(
...
class HSEAnswerDistributionPerCourse(
        HSEAnswerDistributionDownstreamMixin,
        HSEAnswerDistributionPerCourseMixin,
        HSEBaseAnswerDistributionTask):
    """Calculates answer distribution on a problem in a course, given per-user answers by date."""

    def requires(self):
        results = {
            'events': HSEProblemCheckEvent(
                mapreduce_engine=self.mapreduce_engine,
                input_format=self.base_input_format,
                lib_jar=self.lib_jar,
                n_reduce_tasks=self.n_reduce_tasks,
    ...
    def run(self):
        ...
        super(HSEAnswerDistributionPerCourse, self).run()


class HSEAnswerDistributionOneFilePerCourseTask(HSEAnswerDistributionDownstreamMixin, MultiOutputMapReduceJobTask):
    """
    Groups answer distributions by course, producing a different file for each.
    ...
    def requires(self):
        return HSEAnswerDistributionPerCourse(
            mapreduce_engine=self.mapreduce_engine,
            lib_jar=self.lib_jar,
            base_input_format=self.base_input_format,
            n_reduce_tasks=self.n_reduce_tasks,
    ...
    def multi_output_reducer(self, _course_id, values, output_file):
        ...
        This output is visible to instructors, so use an excel friendly format (csv).
        """
        field_names = HSEAnswerDistributionPerCourse.get_column_order()
        writer = csv.DictWriter(output_file, field_names)
        writer.writerow(dict(
            (k, k) for k in field_names
        ))
...
class HSEInsertToMysqlAnswerDistributionTableBase(MysqlInsertTask):
    """
    Define answer_distribution table.
    """
    def rows(self):
        """
        Re-formats the output of HSEAnswerDistributionPerCourse to something insert_rows can understand
        """
        with self.input()['insert_source'].open('r') as fobj:
            for line in fobj:
                course_id, content = line.split('\t')
                ...
                output_list = [
                    ...
                    row_dict['ModuleID'],
                    row_dict['PartID'],
                    row_dict['user_id'],  # new from hse
                    row_dict['username'],  # new from hse
                    row_dict['Time'],  # new from hse
                    row_dict['Correct Answer'],
                    #  row_dict['First Response Count'], removed
                    #  row_dict['Last Response Count'], removed
                    row_dict['ValueID'] if row_dict['ValueID'] else None,
                    row_dict['AnswerValue'] if row_dict['AnswerValue'] else None,
                    try_str_to_float(row_dict['AnswerValue']),
                    row_dict['Variant'] if row_dict['Variant'] else None,
    ...
    @property
    def table(self):
        return "hse_answer_distribution"

    @property
    def columns(self):
        return [
            ('course_id', 'VARCHAR(255) NOT NULL'),
            # ('course_name', 'VARCHAR(255) NOT NULL'),  # new from hse
            ('module_id', 'VARCHAR(255) NOT NULL'),
            # ('module_name', 'VARCHAR(255) NOT NULL'),  # new from hse
            # ('section_id', 'VARCHAR(255) NOT NULL'),  # new from hse
            # ('section_name', 'VARCHAR(255) NOT NULL'),  # new from hse
            # ('subsection_id', 'VARCHAR(255) NOT NULL'),  # new from hse
            # ('subsection_name', 'VARCHAR(255) NOT NULL'),  # new from hse
            # ('vertical_id', 'VARCHAR(255) NOT NULL'),  # new from hse
            # ('vertical_name', 'VARCHAR(255) NOT NULL'),   # new from hse
            # ('part_type_id', 'VARCHAR(255) NOT NULL'),   # new from hse
            ('part_id', 'VARCHAR(255) NOT NULL'),
            ('user_id', 'VARCHAR(255) NOT NULL'),  # new from hse
            ('username', 'VARCHAR(255) NOT NULL'),  # new from hse
            ('time_stamp', 'VARCHAR(255) NOT NULL'),  # new from hse
            ('correct', 'TINYINT(1) NOT NULL'),
            #  ('first_response_count', 'INT(11) NOT NULL'), removed
            #  ('last_response_count', 'INT(11) NOT NULL'), removed
            ('value_id', 'VARCHAR(255)'),
            ('answer_value_text', 'LONGTEXT'),
            ('answer_value_numeric', 'DOUBLE'),
            ('variant', 'INT(11)'),
...
class HSEAnswerDistributionToMySQLTaskWorkflow(
    HSEInsertToMysqlAnswerDistributionTableBase,
    HSEAnswerDistributionDownstreamMixin,
    MapReduceJobTaskMixin
):
...
@workflow_entry_point
class HSEAnswerDistributionWorkflow(
        HSEAnswerDistributionDownstreamMixin,
        MysqlInsertTaskMixin,
        MapReduceJobTaskMixin,
        luigi.WrapperTask):
